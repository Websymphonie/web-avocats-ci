...
final readonly class CreateEditorialVideoHandler implements CommandHandler
{
    ...
    public function __invoke(CreateEditorialVideoCommand $command): EditorialVideo
    {
        ...
        $video = new EditorialVideo(0, '', trim($command->title), $slug, self::clean($command->excerpt), $this->sanitizer->sanitize($command->description), $command->provider, trim($command->videoUrl), category: $category);
