...
use Websymphonie\ContentContext\Domain\Exception\EditorialVideoCategoryRequiredException;
...
{
    public function __construct(private EditorialVideoRepositoryInterface $repository, private TagRepositoryInterface $tagRepository, private EditorialVideoCategoryRepositoryInterface $categoryRepository, private RichTextSanitizerInterface $sanitizer, private SluggerInterface $slugger) {}
    public function __invoke(UpdateEditorialVideoCommand $command): void
    {
        ...
        if ($command->categoryId === null) { throw new EditorialVideoCategoryRequiredException(); }
        $video->update(trim($command->title), $slug ?: $video->slug, self::clean($command->excerpt), $this->sanitizer->sanitize($command->description), $command->provider, trim($command->videoUrl), $this->tagRepository->findByIds($command->tags), $this->categoryRepository->getById($command->categoryId));
