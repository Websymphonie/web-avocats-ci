...
final readonly class BulkDeleteEventsHandler implements CommandHandler
{
    ...
    public function __invoke(BulkDeleteEventsCommand $command): void { foreach ($this->repository->findByIds(array_values(array_unique($command->ids))) as $event) { $this->repository->delete($event); } }
}
