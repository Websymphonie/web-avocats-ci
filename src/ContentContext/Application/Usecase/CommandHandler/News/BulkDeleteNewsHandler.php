...
final readonly class BulkDeleteNewsHandler implements CommandHandler
{
    ...
    public function __invoke(BulkDeleteNewsCommand $command): void { foreach ($this->repository->findByIds(array_values(array_unique($command->ids))) as $news) { $this->repository->delete($news); } }
}
