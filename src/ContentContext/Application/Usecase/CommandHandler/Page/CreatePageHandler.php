...
final readonly class CreatePageHandler implements CommandHandler
{
    ...
    public function __invoke(CreatePageCommand $command): Page
    {
        ...
        try {
            ...
            $page = $this->repository->save(new Page(0, '', trim($command->title), $slug, $this->sanitizer->sanitize($command->content), coverMediaId: $media?->id, group: $command->group, sortOrder: $command->sortOrder));
            $this->personGroups->synchronize($page->id, $command->personGroups);
