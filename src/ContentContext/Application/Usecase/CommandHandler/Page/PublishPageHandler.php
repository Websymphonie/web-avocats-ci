...
final readonly class PublishPageHandler implements CommandHandler
{
    ...
    public function __invoke(PublishPageCommand $command): void
    {
        $page = $this->repository->getById($command->id);
        $page->publish();
        $page = $this->repository->save($page);
        $this->eventPublisher?->publish('PAGE', 'PUBLISHED', $page->uuid, $page->title, $page->slug, $page->publishedAt);
    }
}
