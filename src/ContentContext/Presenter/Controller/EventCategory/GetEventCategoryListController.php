...
final class GetEventCategoryListController extends AbstractController
{
    ...
    public function __invoke(Request $request): Response { $query = new GetEventCategoryListQuery(page: max(1, $request->query->getInt('page', 1))); $form = $this->createForm(EventCategoryFilterType::class, $query, ['method' => 'GET', 'action' => $this->generateUrl('content_admin_event_category_list')]); $form->handleRequest($request); $result = $this->handleQuery(new GetEventCategoryListQuery($query->search ?: null, $query->page, 20)); return $this->render('content/admin/event_category/index.html.twig', ['categories' => $result, 'filterForm' => $form->createView()]); }
}
