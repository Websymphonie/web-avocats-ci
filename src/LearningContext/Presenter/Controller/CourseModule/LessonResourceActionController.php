...
final class LessonResourceActionController extends AbstractController
{
    ...
    public function rename(Request $request, int $trainingId, int $moduleId, int $lessonId, int $resourceId): Response
    {
        ...
        try {
            $this->handleCommand(new UpdateLessonResourceCommand($trainingId, $moduleId, $lessonId, $resourceId, (string)$request->request->get('title')));
            $this->flash()->success('Nom de la ressource modifié.');
    ...
    public function delete(Request $request, int $trainingId, int $moduleId, int $lessonId, int $resourceId): Response
    {
        ...
        try {
            $this->handleCommand(new RemoveLessonResourceCommand($trainingId, $moduleId, $lessonId, $resourceId));
            $this->flash()->success('Ressource retirée de la leçon.');
        } catch (UserFacingError $exception) {
            $this->flash()->errorFromException($exception);
        }
        return $this->redirectToEdit($trainingId, $moduleId, $lessonId);
    ...
    public function reorder(Request $request, int $trainingId, int $moduleId, int $lessonId): Response
    {
        ...
        try {
            $this->handleCommand(new ReorderLessonResourcesCommand($trainingId, $moduleId, $lessonId, $ids));
            $this->flash()->success('Ordre des ressources enregistré.');
        } catch (UserFacingError $exception) {
            $this->flash()->errorFromException($exception);
        }
        return $this->redirectToEdit($trainingId, $moduleId, $lessonId);
