...
final class ReorderCourseModulesController extends AbstractController
{
    ...
    {
        if (!$this->isCsrfTokenValid('learning_course_module_reorder_' . $trainingId, (string)$request->request->get('_token'))) {
            throw $this->createAccessDeniedException('Jeton CSRF invalide.');
        }
        $ids = array_values(array_map('intval', (array)$request->request->all('moduleIds')));
        try {
            $this->handleCommand(new ReorderCourseModulesCommand($trainingId, $ids));
            $this->flash()->success('Ordre des modules enregistré.');
