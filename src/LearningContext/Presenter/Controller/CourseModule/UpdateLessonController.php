...
final class UpdateLessonController extends AbstractController
{
    ...
    public function __invoke(Request $request, int $trainingId, int $moduleId, int $lessonId): Response
    {
        ...
        $lesson = null;
        foreach ($structure->lessons as $item) { if ($item->id === $lessonId) { $lesson = $item; break; } }
        if ($lesson === null) { throw $this->createNotFoundException('Cette leçon est introuvable.'); }
        $command = new UpdateLessonCommand($trainingId, $moduleId, $lesson->id, $lesson->title, $lesson->summary);
        $form = $this->createForm(LessonFormType::class, $command)->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            try { $this->handleCommand($command); $this->flash()->success('Leçon modifiée.'); return $this->redirectToRoute('learning_admin_training_show', ['id' => $trainingId]); } catch (UserFacingError $exception) { $this->flash()->errorFromException($exception); }
        }
        return $this->render('learning/admin/course_structure/lesson_form.html.twig', ['form' => $form->createView(), 'trainingId' => $trainingId, 'module' => $structure->module, 'pageTitle' => 'Modifier la leçon']);
    }
}
