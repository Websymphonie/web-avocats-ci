...
final class CompleteLessonController extends AbstractController
{
    ...
    {
        if (!$this->isCsrfTokenValid('learning_member_lesson_complete_' . $uuid, (string)$request->request->get('_token'))) {
        ...
        try {
            $this->handleCommand(new CompleteLessonCommand($uuid, $user->id));
            $this->flash()->success('Leçon terminée.');
