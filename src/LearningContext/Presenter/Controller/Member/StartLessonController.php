...
final class StartLessonController extends AbstractController
{
    ...
    {
        if (!$this->isCsrfTokenValid('learning_member_lesson_start_' . $uuid, (string) $request->request->get('_token'))) { throw $this->createAccessDeniedException('Jeton CSRF invalide.'); }
        ...
        try { $this->handleCommand(new StartLessonCommand($uuid, $user->id)); $this->flash()->success('Leçon commencée.'); } catch (UserFacingError $exception) { $this->flash()->errorFromException($exception); }
