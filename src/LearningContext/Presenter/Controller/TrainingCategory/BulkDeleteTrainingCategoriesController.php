...
final class BulkDeleteTrainingCategoriesController extends AbstractController
{
    /**
    ...
    {
        if (!$this->isCsrfTokenValid('training-category-bulk-delete', (string)$request->request->get('_token'))) {
        ...
        if ($ids !== []) {
            $deleted = $this->handleCommand(new BulkDeleteTrainingCategoriesCommand($ids));
            $this->flash()->success(sprintf('%d catégorie(s) supprimée(s). Les catégories utilisées ont été conservées.', $deleted));
        }
