...
final class UpdateTrainingCategoryController extends AbstractController
{
    public function __construct(private readonly TrainingCategoryRepositoryInterface $repository)
    {
    }
    ...
    public function __invoke(Request $request, int $id): Response
    {
        ...
        $command = new UpdateTrainingCategoryCommand($category->id, $category->name);
        $form = $this->createForm(TrainingCategoryFormType::class, $command);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $this->handleCommand($command);
                $this->flash()->success('Catégorie de formation modifiée.');
        ...
        return $this->render('learning/admin/taxonomy/form.html.twig', ['form' => $form->createView(), 'title' => 'Modifier la catégorie de formation', 'backRoute' => 'learning_admin_category_list']);
    }
}
