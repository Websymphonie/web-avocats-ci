...
final class BulkDeleteTrainingTagsController extends AbstractController
{
    /**
    ...
    {
        if (!$this->isCsrfTokenValid('training-tag-bulk-delete', (string)$request->request->get('_token'))) {
        ...
        if ($ids !== []) {
            $deleted = $this->handleCommand(new BulkDeleteTrainingTagsCommand($ids));
            $this->flash()->success(sprintf('%d tag(s) supprimé(s). Les tags utilisés ont été conservés.', $deleted));
        }
