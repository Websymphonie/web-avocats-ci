...
final class CreateTrainingTagController extends AbstractController
{
    /**
    ...
    {
        $command = new CreateTrainingTagCommand();
        $form = $this->createForm(TrainingTagFormType::class, $command);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $this->handleCommand($command);
                $this->flash()->success('Tag de formation créé.');
        ...
        return $this->render('learning/admin/taxonomy/form.html.twig', ['form' => $form->createView(), 'title' => 'Nouveau tag de formation', 'backRoute' => 'learning_admin_tag_list']);
    }
}
