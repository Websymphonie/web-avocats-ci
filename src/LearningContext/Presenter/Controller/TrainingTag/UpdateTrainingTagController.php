...
final class UpdateTrainingTagController extends AbstractController
{
    public function __construct(private readonly TrainingTagRepositoryInterface $repository)
    {
    }
    ...
    public function __invoke(Request $request, int $id): Response
    {
        ...
        $command = new UpdateTrainingTagCommand($tag->id, $tag->name);
        $form = $this->createForm(TrainingTagFormType::class, $command);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $this->handleCommand($command);
                $this->flash()->success('Tag de formation modifié.');
        ...
        return $this->render('learning/admin/taxonomy/form.html.twig', ['form' => $form->createView(), 'title' => 'Modifier le tag de formation', 'backRoute' => 'learning_admin_tag_list']);
    }
}
